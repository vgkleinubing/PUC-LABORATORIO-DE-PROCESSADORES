...
1.30 2008-06-25    M. Stemmer: Added entry to the device table to support LPC2378
1.32 2009-05-18    M. Stemmer: New NXP lpx23xx series part ID's
******************************************************************************/

#define VERSION_STR	"1.32"
...
/* Acrescentado por Stemmer para fazer o 2378 funcionar */
  { 0x0603FB02, 2364, 128, 16, 11, 4096, SectorTable_213x },
...
/* In 2008 manual NXP assigned new part ID's for LPC23xx series */
  { 0x1600F902, 2364, 128, 8, 11, 4096, SectorTable_213x },
  { 0x1600F823, 2365, 256, 32, 15, 4096, SectorTable_213x },
  { 0x1600F923, 2366, 256, 32, 15, 4096, SectorTable_213x },
  { 0x1600F825, 2367, 512, 32, 27, 4096, SectorTable_213x },
  { 0x1600F925, 2368, 512, 32, 27, 4096, SectorTable_213x },
  { 0x1700F825, 2377, 512, 32, 27, 4096, SectorTable_213x },
  { 0x1700FD25, 2378, 512, 32, 27, 4096, SectorTable_213x },
  { 0x1800F935, 2387, 512, 64, 27, 4096, SectorTable_213x },
  { 0x1700FF35, 2388, 512, 64, 27, 4096, SectorTable_213x },
  { 0x1500FF35, 2458, 512, 64, 27, 4096, SectorTable_213x },
  { 0x1600FF35, 2468, 512, 64, 27, 4096, SectorTable_213x },
    ...
    if(IspEnvironment->FormatHex != 0)
    {
        ...
        while(Pos < FileLength)
        {
            ...
            else if(RecordType == 0x04)     // 04 - Extended linear address record, used by IAR
            {
				...
				{
					DebugPrintf(1, "Use of multiple Extended linear address records [05] not supported\n");
					exit(1);
				}
                for(i = 0; i < RecordLength * 2; i++)   // double amount of nibbles
                {
                ...
                RealAddress <<= 16;
                IspEnvironment->BinaryOffset = RealAddress & 0xffff0000; // set startaddress of BinaryContent
            }
...
int PhilipsDownload(ISP_ENVIRONMENT *IspEnvironment)
{
    ...
    *endPtr = '\0'; /* delete \r\n */
    ...
    DebugPrintf( 2, " (%s)\n", strippedAnswer);
    //IspEnvironment->DetectedDevice = 25;
...
int main(int argc, char *argv[])
{
    ...
    if(IspEnvironment.StartAddress == 0)
    {
